...
[cols="10%,10%,10%,70%", width="100%, options="header"]
|===
|Property | Type | Default value | Description

| *noImports* | `boolean` | `false` | Only generate code for the WSDL document that appears on the command line if this value is true. The default behaviour is to generate files for all WSDL documents, the immediate one and all imported ones. +
...
Default is an empty string. Assumes server-side="true". WSDL2Java Parameter: -S, --skeletonDeploy <true or false>
...
=== Axis 2
The base configuration uses dependencies of Axis 2 with version 1.7.7
